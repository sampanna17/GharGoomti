...
function Slider({ images = [] }) {
  ...
  return (
    <div className="w-full h-[450px] flex gap-5 sm:h-[380px]">
      ...
      {imageIndex !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-md flex items-center justify-center z-50">
          <div className="relative p-4 rounded-lg flex flex-col items-center transform transition-transform duration-300 scale-y-100"
            style={{ transform: imageIndex === null ? "scaleY(0)" : "scaleY(1)" }}>
            <button className="absolute -top-4 right-16 text-gray-600 text-xl font-bold"
              onClick={() => setImageIndex(null)}>
              <img src={Cross} alt="Close" className="w-5 h-auto filter invert" />
            </button>
            <div className="relative flex items-center">
              <button className="left-2 p-2 filter invert" onClick={() => changeSlide("left")}>
                <img src={LeftArrow} alt="Left" className="w-9 h-10 opacity-80 hover:opacity-100" />
              </button>
              <img src={images[imageIndex].imageURL} alt="Preview" className="w-[600px] h-[500px] object-cover rounded-lg" />
              <button className="right-2 p-2 filter invert" onClick={() => changeSlide("right")}>
                <img src={RightArrow} alt="Right" className="w-8 opacity-80 hover:opacity-100" />
              </button>
            </div>
          </div>
      ...
      <div className={`flex ${isSixImages ? 'flex-col' : (isFiveImages ? 'flex-col' : 'flex-row')} gap-5 sm:h-[380px]`}>
        ...
        <div className={`flex ${isSixImages ? 'flex-row gap-4 w-full' : (images.length === 4 ? 'flex-1 flex flex-col justify-between gap-2 sm:gap-1' : (isFiveImages ? 'grid grid-cols-3 gap-4 w-full' : 'flex-1 flex-col justify-between gap-4 sm:gap-2'))}`}>
          {isSixImages ? (
            <>
              <img src={images[3].imageURL} alt="" className="h-[200px] sm:h-[160px] w-[calc(33.33%-10px)] object-cover rounded-lg cursor-pointer" onClick={() => setImageIndex(3)} />
              <img src={images[4].imageURL} alt="" className="h-[200px] sm:h-[160px] w-[calc(33.33%-10px)] object-cover rounded-lg cursor-pointer" onClick={() => setImageIndex(4)} />
              <img src={images[5].imageURL} alt="" className="h-[200px] sm:h-[160px] w-[calc(33.33%-10px)] object-cover rounded-lg cursor-pointer" onClick={() => setImageIndex(5)} />
            </>
          ) : (
            images.slice(isFiveImages ? 2 : 1).map((image, index) => (
              <img 
